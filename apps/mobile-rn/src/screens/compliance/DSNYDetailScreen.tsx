...
const DSNYDetailScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        setItems(violations.filter(v => v.type === 'DSNY_VIOLATION'));
      } catch (e: any) {
        setError(e?.message || 'Failed to load DSNY violations');
      } finally {
        setLoading(false);
      }
    })();
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>DSNY Violations ({items.length})</Text>
      <FlatList
        data={items}
        keyExtractor={(item) => String(item.id)}
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>{item.title || 'Summons'}</Text>
            <Text style={styles.cardSub}>Status: {item.status}</Text>
            {item.fineAmount != null && (
              <Text style={styles.cardSub}>Fine: ${item.fineAmount}</Text>
            )}
            {item.dateIssued && (
...
export default DSNYDetailScreen;
